...
30초마다 변경된 게임 상태를 자동 저장한다. 명시적 저장과 종료 저장도 유지한다.
게임 서버 강제 종료 시 마지막 DB 확인 이후의 변경은 손실될 수 있다. API/DB 장애 중에는 30초 손실 한도를 보장하지 않는다.
자동 저장 완료 후 계속 움직이는 플레이어를 매 HTTP 응답마다 재저장하지 않고, 다음 자동/명시적 저장에서 반영한다.
사용권 갱신 확인이 90초 동안 없으면 플레이를 차단하고 복구 데이터를 남긴다. 연결이 끊긴 세션의 최종 정리는 최대 60초 동안 시도한 뒤 사용권을 계속 갱신하지 않는다.
...
- 재접속 시 같은 서버 디스크에 해당 캐릭터의 복구 파일이 하나 있으면, 새 사용권을 획득한 뒤 기존 request_id/expected_revision으로 재처리한다. DB 현재 revision이 영수증 revision과 같은지 확인한 뒤 최신 스냅샷까지 반영하고 파일을 제거한다.
- 복구 파일이 여러 개이거나 버전 충돌/잘못된 파일이면 자동 선택 또는 강제 덮어쓰기 없이 접속을 차단한다. 운영자가 DB와 파일을 비교해야 한다. 다른 서버로 이동하려면 복구 디스크도 전달해야 한다. 디스크 장애·강제 종료 직전의 미수집 변경은 보장하지 않는다.
- PIE Esc/Stop은 월드 파괴 전 HTTP 완료를 최대 5초 기다린다. 이는 에디터 전용이며 강제 프로세스 종료를 처리하는 수단은 아니다. 정상 운영 종료는 플레이어 최종 저장 및 사용권 해제 완료 후 실행한다. 별도의 서버 종료 명령 자동화는 이 변경에 포함하지 않는다.
