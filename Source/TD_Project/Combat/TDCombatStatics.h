...
public:
	...
	* @param DamageScale  공격력 배율. 보스 패턴·스킬이 쓴다. 평타는 1.
	*/
	static FTDDamageResult ApplyDamage(AActor* Attacker, AActor* Target,
		const FGameplayTagContainer& ContextTags, float DamageScale = 1.f);

	/** 박스 안의 유효 대상(적팀·생존자·공격자 제외). 서버 전용. 평타와 보스 패턴이 공유한다. */
