...
struct FTDItemRow : public FTableRowBase
{
	...
	/**
	 * NPC 상점에 되팔 때의 기준값. 실제로 받는 돈은 여기에 상점 비율(기본 0.25)을 곱한 것이다.
	 *
	 * **상점이 파는 가격(DT_ShopItem.Price)과 다른 값이다.** 판매가는 아이템의 값어치라
	 * 어느 마을에서 팔든 같지만, 구매가는 상점이 정하므로 마을마다 다를 수 있다.
	 * 처음에는 되팔기를 구매가에 묶었는데, 그러면 상점이 팔지 않는 물건 —
	 * 즉 몬스터·보스 드롭 전부 — 을 팔 수 없게 되어 나눴다.
	 *
	 * 0 이면 0 원에 팔린다. 거절하지 않는 이유는 잡템을 비우는 통로가 필요해서다.
	 * 다만 장신구가 0 인 것은 대개 값을 안 채운 것이라 검증 스크립트가 경고한다.
	 *
	 * 강화 레벨은 반영하지 않는다. 강화한 물건은 거래소에서 사람에게 파는 것이 전제다.
	 */
