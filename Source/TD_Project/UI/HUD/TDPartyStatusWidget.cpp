...
{
	if (UWorld* World = GetWorld(); World && !World->GetTimerManager().IsTimerActive(
			PartyChangeRefreshTimer)){
		// 같은 프레임의 여러 복제 알림을 모은 뒤 완성된 상태로 한 번 갱신한다.
		PartyChangeRefreshTimer = World->GetTimerManager().SetTimerForNextTick(
				this, &ThisClass::RefreshParty);
	}
}
