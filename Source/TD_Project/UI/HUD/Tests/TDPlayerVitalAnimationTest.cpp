...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FTDPlayerVitalAnimationTest,
 "TD.UI.PlayerStatus.VitalAnimation", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTDPlayerVitalAnimationTest::RunTest(const FString& Parameters)
{
 auto Update = [](FTDPlayerVitalAnimation& State, float Current, float Maximum, bool bHP, bool bAnimate = true)
 {
  State.SetTarget(Current, Maximum, Maximum > 0.f ? Current / Maximum : 0.f,
   bAnimate, bHP, 0.2f, 0.25f, 0.18f, 0.4f, 0.35f);
 };
 FTDPlayerVitalAnimation HP;
 ...
 HP.Advance(0.28f);
 TestTrue(TEXT("Trail moves after delay including frame overshoot"), HP.TrailPercent > HP.DisplayPercent && HP.TrailPercent < 875.f / 975.f);
 ...
 HP.Advance(0.125f);
 TestTrue(TEXT("Recovery smoothly fills"), HP.DisplayPercent > 275.f / 975.f && HP.DisplayPercent < 675.f / 975.f);
 ...
 MP.Advance(0.1f);
 TestTrue(TEXT("Mana is interpolated"), MP.DisplayPercent > 150.f / 320.f && MP.DisplayPercent < 250.f / 320.f);
